...
# reflex agent에 manhattan distace를 응용한 baseline2.
class MyAgent_2(CaptureAgent):
...
class MyOffense(MyAgent_2):
  ...
  def getFeatures(self, gameState, action):
    ...
    # 유령까지의 거리에 manhattanDistance를 heuristic으로 도입.
    nextGDist = [manhattanDistance(nextCurr, g.getPosition()) for g in nextGhost]
