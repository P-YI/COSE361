...
# reflex agent에 myHeuristic을 응용한 baseline3.
class MyAgent_3(CaptureAgent):
...
class MyOffense(MyAgent_3):
  ...
  def getFeatures(self, gameState, action):
    ...
    # 유령까지의 거리에 myHeuristic을 heuristic으로 도입.
    nextGDist = [myHeuristic(nextCurr, g.getPosition()) for g in nextGhost]
...
def myHeuristic(xy1, xy2):
    # manhattan과 euclidean의 중간 격으로, euclidean의 길이를 바탕한 원주의 절반을 heuristic으로 활용.
  from math import pi
